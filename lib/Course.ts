...
class Course {
   ...
   */
  createMetaFile(buildPath):Q.Promise<any> {
    var self = this;
    ...
    try {
      ...
      metaFileStream.on('close', () => {
        deferred.resolve(metaDataFilePath);
      });

      var xmlWriter = new XMLWriter(true, (content, encoding) => {
        metaFileStream.write(content, encoding);
      });

      // write head of the document
      var xmlCourse = xmlWriter.startDocument('1.0', 'utf-8')
        .startElement('course')
      ...
      self.modules.forEach((element, index) => {
        var moduleName = self.id + '-m' + index;

        // todo: remove hard coded author
        // add module
        xmlCourse.startElement('module')
          .writeAttribute('author', 'andrew-connell')
          .writeAttribute('name', moduleName)
          .endElement();
      });
   ...
   * @returns {boolean}         Flag indicating the title is valid.
   */
  isValid():boolean {
    var self = this;
    var valid = true;
    ...
    if (!self.title || self.title.length > 65) {
      valid = false;
    }
   ...
   */
  static getFullPath(rootPath:string, courseId:string):string {
    var result:string = path.join(rootPath, courseId);
   ...
   */
  static loadFromYaml(fullPath:string):Q.Promise<Course> {
    var deferred = Q.defer();
   ...
   */
  static loadFromMeta(fullPath:string):Q.Promise<Course> {
    var deferred = Q.defer();
